        ...
        for index in batch_memory_index:
            ...
            for k in range(1, min(self.horizon_K, len(self.memory) - index - 1) + 1):
                ...
                if self.memory[index + k][4]:
                    ...
                    q_eval = self.brain.predict_eval_action([self.memory[index + k][3]])
                    ...
                    # print('q_next', q_next)
                    max_index = np.argmax(q_eval)
                    q_lower.append(cumulative_r + math.pow(self.gamma, k + 1) * q_next[0][max_index])
            if q_lower:
    ...
    def calculating_q_upper_min(self, batch_memory_index):
        ...
        for index in batch_memory_index:
            ...
            for k in range(1, min(self.horizon_K, index - 1) + 1):
                ...
                q_eval = self.brain.predict_eval_action([memory_before[0]])
                max_index = np.argmax(q_eval)
                ...
                q_upper.append(math.pow(self.gamma, -k - 1) * q_before[0][max_index] - cumulative_r)
            if q_upper:
