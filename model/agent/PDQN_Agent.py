...
- gym: 0.9.2
"""
from .DQN_Agent import DQN_Agent as agent
import numpy as np
import random
import math
...
class PDQN_Agent(agent):
    def __init__(
        ...
        super(PDQN_Agent, self).__init__(brain, observation_space_shape, n_actions,  reward_decay, MAX_EPSILON,
    ...
    def calculating_q_lower_max(self, batch_memory_index):
        ...
        for k in range(1, self.horizon_K + 1):
            batch_memory = [self.memory[index + k][3] if (index + k) < len(self.memory) else self.memory[0][3] for index in batch_memory_index]
    ...
    def calculating_q_upper_min(self, batch_memory_index):
        ...
        q_before_list = []
        for k in range(1, self.horizon_K + 1):
            batch_memory = [self.memory[index - k - 1][0] if (index - k - 1) > -1 else self.memory[0][0] for index in batch_memory_index]
    ...
    def learn(self, costFlag=False):
        ...
        batch_memory_index = np.array(random.sample(range(len(self.memory)), batch_size))
