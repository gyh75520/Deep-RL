...
import numpy as np
from .DQN_PER_Brain import DQN_PER_Brain as Brain


class PDQN_RePER_Brain(Brain):
    def __init__(
        self,
        ...
        super(PDQN_RePER_Brain, self).__init__(n_actions, n_features, neurons_per_layer, activation_function,
                                               Optimizer, learning_rate, w_initializer, b_initializer, output_graph, restore, checkpoint_dir)
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('loss'):
            ...
            self.loss = tf.reduce_mean(tf.multiply(self.ISWeights, tf.squared_difference(self.q_target, Q_action)))
            if self.output_graph:
